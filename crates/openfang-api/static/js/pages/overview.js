  ...
  return {
    ...
    async loadOverview() {
      ...
      try {
        ...
        this.loadError = e.message || t('overview_connection_error');
      }
      this.loading = false;
    },
    ...
    providerTooltip(p) {
      if (p.health === 'cooldown') return t('overview_tooltip_cooldown', { name: p.display_name });
      if (p.health === 'open') return t('overview_tooltip_open', { name: p.display_name });
      if (p.auth_status === 'configured') return t('overview_tooltip_ready', { name: p.display_name });
      return t('overview_tooltip_not_configured', { name: p.display_name });
    },
    ...
    get setupChecklist() {
      return [
        { key: 'provider', label: t('overview_check_provider'), done: this.configuredProviders.length > 0, action: '#settings' },
        { key: 'agent', label: t('overview_check_agent'), done: (Alpine.store('app').agents || []).length > 0, action: '#agents' },
        { key: 'chat', label: t('overview_check_chat'), done: localStorage.getItem('of-first-msg') === 'true', action: '#chat' },
        { key: 'channel', label: t('overview_check_channel'), done: this.channels.length > 0, action: '#channels' },
        { key: 'skill', label: t('overview_check_skill'), done: localStorage.getItem('of-skill-browsed') === 'true', action: '#skills' }
      ];
    },
    ...
    timeAgo(timestamp) {
      ...
      if (diff < 10) return t('time_just_now');
    ...
    friendlyAction(action) {
      if (!action) return t('overview_action_unknown');
      var map = {
        'AgentSpawn': t('overview_action_agent_created'),
        'AgentKill': t('overview_action_agent_stopped'),
        'AgentTerminated': t('overview_action_agent_stopped'),
        'ToolInvoke': t('overview_action_tool_used'),
        'ToolResult': t('overview_action_tool_completed'),
        'MessageReceived': t('overview_action_message_in'),
        'MessageSent': t('overview_action_response_sent'),
        'SessionReset': t('overview_action_session_reset'),
        'SessionCompact': t('overview_action_compacted'),
        'ModelSwitch': t('overview_action_model_changed'),
        'AuthAttempt': t('overview_action_login_attempt'),
        'AuthSuccess': t('overview_action_login_ok'),
        'AuthFailure': t('overview_action_login_failed'),
        'CapabilityDenied': t('overview_action_denied'),
